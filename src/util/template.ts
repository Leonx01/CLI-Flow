...
import { isRecord, log } from './utils.js';
 ...
 *   replace(old,new), keys, length, first, last, array, json
 */
function applyFilter(filterExpr: string, value: unknown): unknown {
  ...
  switch (name) {
    ...
    case 'array':
      // Normalizes a value that may be a scalar or an array into always-an-array —
      // e.g. a single-select vs multi-select interact result — so downstream
      // args expecting a list work regardless of which shape upstream produced.
...
function evalJsExpr(expr: string, ctx: RenderContext): unknown {
  ...
  try {
    ...
    log.debug?.(`Template expression failed: "${expr}" → ${err instanceof Error ? err.message : String(err)}`);
    return undefined;
  }
}
